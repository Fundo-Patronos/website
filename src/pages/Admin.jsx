...
import { ArrowLeftOnRectangleIcon, ExclamationTriangleIcon, CheckCircleIcon } from '@heroicons/react/24/outline'
...
function CategoriaBadge({ value }) {
  ...
  if (value === 'Associado') {
    return (
      <span
        className="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold bg-white"
        style={{ borderImage: `${BRAND_GRADIENT} 1`, borderStyle: 'solid', color: '#ff6253' }}
      >
        Associado
      </span>
    )
      ...
      <table className="min-w-full divide-y divide-gray-200 text-sm">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left font-semibold text-gray-700">Assinatura Status</th>
          </tr>
        </thead>
...
// ---------- TAB: Adicionar PIX ----------
function AddPixTab({ getToken, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const r = await fetch('/api/admin/donations', {
        ...
        body: JSON.stringify({ email, amount, occurredAt: date, source: 'pix' }),
      })
      const data = await r.json()
      if (!r.ok) throw new Error(data.error || 'Erro ao salvar')
      setFeedback({ type: 'success', msg: `PIX de ${formatCurrency(parseFloat(amount))} registrado para ${email}.` })
      setEmail(''); setAmount(''); setDate(new Date().toISOString().split('T')[0])
      onSuccess?.()
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-xl">
      <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-gray-200 bg-white p-6">
        <h3 className="text-lg font-semibold text-gray-900">Registrar doação PIX</h3>
        <p className="text-sm text-gray-600">Cada PIX vira um evento imutável. A categoria e o valor total do doador são recalculados automaticamente.</p>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Data da doação</label>
          <input
            type="date"
            required
            value={date}
        ...
        <button
          ...
          style={{ background: BRAND_GRADIENT }}
        >
          {loading ? 'Salvando...' : 'Registrar PIX'}
        </button>
      </form>
    </div>
  )
...
// ---------- TAB: Cadastrar Perfil ----------
function AddProfileTab({ getToken, onSuccess }) {
  const [email, setEmail] = useState('')
  const [nome, setNome] = useState('')
  const [tipoContribuicao, setTipoContribuicao] = useState('Pontual')
  const [estadoAssinatura, setEstadoAssinatura] = useState('N/A')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const r = await fetch('/api/admin/donors', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ email, nome, tipoContribuicao, estadoAssinatura, valorAssinatura: parseFloat(valorAssinatura) || 0 }),
      })
      const data = await r.json()
      if (!r.ok) throw new Error(data.error || 'Erro ao salvar')
      setFeedback({ type: 'success', msg: `Perfil de ${nome} (${email}) salvo.` })
      setEmail(''); setNome(''); setTipoContribuicao('Pontual'); setEstadoAssinatura('N/A'); setValorAssinatura('0')
      onSuccess?.()
    } catch (err) {
      setFeedback({ type: 'error', msg: err.message })
    } finally {
  ...
  return (
    <div className="mx-auto max-w-xl">
      <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-gray-200 bg-white p-6">
        <h3 className="text-lg font-semibold text-gray-900">Cadastrar / atualizar perfil de doador</h3>
        <p className="text-sm text-gray-600">Esses campos vêm do registro manual. Valor total e categoria são computados a partir dos eventos.</p>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={tipoContribuicao} onChange={(e) => setTipoContribuicao(e.target.value)}
              className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500">
          ...
          <div>
            ...
            <select value={estadoAssinatura} onChange={(e) => setEstadoAssinatura(e.target.value)}
              className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500">
              <option>N/A</option>
              <option>Ativa</option>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Valor da assinatura (R$/mês — 0 se não tem)</label>
          <input type="number" step="0.01" min="0" value={valorAssinatura} onChange={(e) => setValorAssinatura(e.target.value)}
            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500" />
        </div>
...
function RulesTab({ getToken, onChanged }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await r.json()
      if (!r.ok) throw new Error(data.error || 'Erro')
      setFeedback({ type: 'success', msg: 'Regras atualizadas. Categorias dos doadores são recalculadas automaticamente.' })
      setRules(data)
      onChanged?.()
    } catch (err) {
      setFeedback({ type: 'error', msg: err.message })
  ...
  return (
    <div className="mx-auto max-w-xl">
      <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-gray-200 bg-white p-6">
        ...
        <p className="text-sm text-gray-600">A categoria de cada doador é <strong>computada</strong> a partir do valor total doado:</p>
        <ul className="ml-5 list-disc text-sm text-gray-600">
          ...
          <li><strong>Associado</strong> se ≥ <code className="bg-gray-100 px-1">min_associado</code> mas &lt; <code className="bg-gray-100 px-1">min_patrono</code></li>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" step="0.01" min="0.01" required value={minPatrono} onChange={(e) => setMinPatrono(e.target.value)}
              className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">min_associado (R$)</label>
            <input type="number" step="0.01" min="0.01" required value={minAssociado} onChange={(e) => setMinAssociado(e.target.value)}
              className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500" />
          </div>
        </div>
  ...
  return (
    <Shell signOut={signOut}>
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        {activeTab === 'donors'      && <DonorsTab donors={donors} loading={donorsLoading} />}
        {activeTab === 'add-pix'     && <AddPixTab getToken={getToken} onSuccess={fetchDonors} />}
        {activeTab === 'add-profile' && <AddProfileTab getToken={getToken} onSuccess={fetchDonors} />}
        {activeTab === 'rules'       && <RulesTab getToken={getToken} onChanged={fetchDonors} />}
      </div>
    </Shell>
  )
}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        <div className="inline-block h-12 w-12 animate-spin rounded-full border-4 border-solid border-r-transparent" style={{ borderColor: '#ff9700 transparent #ff9700 #ff9700' }}></div>
